...
async fn test_list_create_perm() {
    ...
    let list_id = dao::create_list(&mut conn, &user, change.clone())
        .await
        .unwrap();
    ...
    let list1 = dao::create_list(&mut conn, &second_user, list1_content.clone())
        .await
        .unwrap();
    let list2 = dao::create_list(&mut conn, &second_user, list2_content.clone())
        .await
        .unwrap();
    // shouldn't be visible for user
    let list3 = dao::create_list(&mut conn, &second_user, list3_content.clone())
        .await
        .unwrap();
    ...
    {
        let res = dao::list_sharing(&mut conn, &second_user, &list1)
            .await
            .unwrap();
        assert_eq!(res.len(), 1);
    ...
    {
        let res = dao::list_sharing(&mut conn, &second_user, &list2)
            .await
            .unwrap();
        assert_eq!(res.len(), 1);
        let entry = res.get(&user.0).unwrap();
...
async fn test_list_change() {
    ...
    let l1_id = dao::create_list(&mut conn, &user, l1.clone())
        .await
        .unwrap();
    ...
    dao::change_list(&mut conn, &user, l1_id.clone(), change.clone())
        .await
        .unwrap();
    ...
    // delete
    dao::delete_list(&mut conn, &user, l1_id.clone())
        .await
        .unwrap();
...
async fn test_entry_change() {
    ...
    let e1_id = dao::create_entry(&mut conn, user.clone(), l1_id.clone(), e1.clone())
        .await
        .unwrap();
    ...
    dao::change_entry(&mut conn, &user, e1_id.clone(), change.clone())
        .await
        .unwrap();
    ...
    //# delete
    dao::delete_entry(&mut conn, &user, e1_id.clone())
        .await
        .unwrap();
