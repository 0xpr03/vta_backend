...
fn gen_meanings(rng: &mut ThreadRng, amount: usize) ->  Vec<EntryMeaning> {
    (0..amount).into_iter().map(|_|EntryMeaning {
...
async fn get_deleted_lists(sql: &mut DbConn, user: &UserId) -> Vec<Uuid> {
    let ret: Vec<Uuid> = sqlx::query_scalar::<_,Uuid>("SELECT list FROM deleted_list WHERE user = ?")
    .bind(user.0)
    .fetch(sql).try_collect().await.unwrap();
    ret
}

async fn get_deleted_entries(sql: &mut DbConn, list: &ListId) -> Vec<Uuid> {
    let ret: Vec<Uuid> = sqlx::query_scalar::<_,Uuid>("SELECT `entry` FROM deleted_entry e WHERE e.list = ?")
    .bind(list.0)
    .fetch(sql).try_collect().await.unwrap();
    ret
}
...
async fn list_updated_date(sql: &mut MySqlConnection, list: &ListId) -> Timestamp {
    sqlx::query_scalar::<_,Timestamp>("SELECT updated FROM lists WHERE uuid = ?").bind(list.0).fetch_one(sql).await.unwrap()
}
...
async fn entry_updated_date(sql: &mut MySqlConnection, entry: &EntryId) -> Timestamp {
    sqlx::query_scalar::<_,Timestamp>("SELECT updated FROM entries WHERE uuid = ?")
        .bind(entry.0).fetch_one(sql).await.unwrap()
}