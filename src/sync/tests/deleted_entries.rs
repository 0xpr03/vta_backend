...
async fn test_basic_deleted_lists() {
    ...
    // delete request
    let mut rng = rand::thread_rng();
    let v = EntryDeletedRequest{ since: None, entries: vec![
        EntryDeleteEntry { list: list1.uuid.clone(), entry: entries1[0].uuid }, // ok
        EntryDeleteEntry { list: list2.uuid.clone(), entry: entries2[0].uuid.clone() },// no perms
        EntryDeleteEntry { list: list3.uuid.clone(), entry: entries3[0].uuid.clone() },// read only
        EntryDeleteEntry { list: list1.uuid.clone(), entry: Uuid::new_v4() },// unknown
    ]};
    ...
    let empty_req = EntryDeletedRequest{since: None, entries: vec![]};
    ...
    let del_shared = EntryDeletedRequest{ since: None, entries: vec![
        EntryDeleteEntry { list: list3.uuid.clone(),entry: entries3[0].uuid.clone() }
    ]};
