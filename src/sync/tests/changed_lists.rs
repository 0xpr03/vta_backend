...
async fn test_basic_changed_lists() {
    ...
    assert_eq!(0,res.failures.len());
    assert_list_eq(&change_req.lists[0], res.lists.get(&change_req.lists[0].uuid).expect("list not found"),ListPermissions::Owner);
    assert_list_eq(&change_req.lists[1], res.lists.get(&change_req.lists[1].uuid).expect("list not found"),ListPermissions::Owner);
    ...
    assert_eq!(0,res.failures.len());
    assert_list_eq(&change_req.lists[0], res.lists.get(&change_req.lists[0].uuid).expect("list not found"),ListPermissions::Read);
    ...
    assert_eq!(1,res.lists.len());
    assert_eq!(0,res.failures.len());
    assert_eq!(change_req.lists[0].name,res.lists[&change_req.lists[0].uuid].name);
