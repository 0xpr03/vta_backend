...
async fn test_basic_changed_lists() {
    ...
    let change_empty_d = ListChangedRequest {since: Some(time1),lists: vec![]};
    let res = dao::update_changed_lists(&mut conn, change_empty_d, &user).await.unwrap();
    assert_eq!(0,res.delta.len());
    assert_eq!(0,res.failures.len());

    let change_empty = ListChangedRequest {since: None,lists: vec![]};
    ...
    insert_list_perm(&mut conn,&second_user,&change_req.lists[0].uuid,false,true).await;
    ...
    let mut change_unperm = ListChangedRequest {since: Some(time2),lists: vec![change_req.lists[0].clone()]};
