...
async fn test_basic_changed_entries() {
    ...
    let entries2: Vec<_> = (0..5)
        .into_iter()
        .map(|_| gen_entry(&list2.uuid, None))
        .collect();

    let entries: Vec<EntryChangedEntry> = entries1
        .iter()
        .chain(entries2.iter())
        .map(|v| v.clone())
        .collect();
    ...
    let resp = dao::update_changed_entries(&mut conn, data, &user)
        .await
        .unwrap();
    ...
    for e_exp in entries1.iter() {
        ...
        let e_res = resp
            .delta
            .get(&e_exp.uuid)
            .expect("inserted entry not found");
    ...
    let resp = dao::update_deleted_entries(&mut conn, del_req, &user)
        .await
        .unwrap();
    assert_eq!(resp.ignored.len(), 0);
    assert_eq!(resp.invalid.len(), 0);
...
async fn test_basic_changed_entries_shared() {
    ...
    let data = EntryChangedRequest {
        since: None,
        entries: entries.iter().map(|v| (*v).clone()).collect(),
    };
