...
pub async fn user_key(sql: &mut MySqlConnection, user: &UserId) -> Result<Option<UserKeyParsed>> {
    if let Some(raw) = sqlx::query_as::<_,UserKey>("SELECT user_id,auth_key,key_type FROM user_key WHERE user_id = ?")
        .bind(user.0).fetch_optional(&mut *sql).await.context("retrieving user key")? {
...
pub async fn user_deleted(sql: &mut MySqlConnection, user: &UserId) -> Result<bool> {
    let res = sqlx::query_as::<_,(bool,)>("SELECT 1 FROM deleted_user WHERE user = ?")
        .bind(user.0).fetch_optional(sql).await?;
    Ok(res.is_some())
}
...
pub async fn create_password_login(sql: &mut MySqlConnection,user: &UserId,email: &str,password: &str) -> Result<()> {
    
    let sql_pw_bind = "INSERT INTO user_login (user_id,email,password,verified) VALUES(?,?,?,?)";
    let res = sqlx::query(sql_pw_bind).bind(user.0).bind(email).bind(password).bind(false).execute(sql).await;
    if check_duplicate(res)? {
...
pub async fn user_by_uuid(sql: &mut MySqlConnection, user: &UserId) -> Result<Option<User>> {
    ...
    let user: Option<User> = sqlx::query_as(sql_fetch).bind(user.0).fetch_optional(sql).await?;
    Ok(user)
}
...
pub async fn delete_user(sql: &mut MySqlConnection, user: &UserId) -> Result<()> {
    ...
    if sqlx::query_as::<_,(bool,)>("SELECT 1 from users WHERE uuid = ?")
        .bind(user.0).fetch_optional(&mut transaction).await?.is_none() {
    ...
    WHERE l.owner = ?";
    let res = sqlx::query(sql_lists_shared).bind(t_now).bind(&user.0).execute(&mut transaction).await?;
    trace!(affected=res.rows_affected(),"creating shared list tombstones");
    // delete user
    let sql_del = "DELETE FROM users WHERE uuid = ?";
    let res = sqlx::query(sql_del).bind(&user.0).execute(&mut transaction).await?;
